...
$line = array(NULL, '1-я фабрика', '2-я фабрика');
$namber_base = array(NULL, 'otk_ru.ru_mtn.RudaS_WM_OF1', 'otk_ru.ru_mtn.RudaS_WM_OF2');
$day_interval = array();
$first_year = 2008;
$last_year = date("Y"); 
$Year_interval = array();
...
function construction_interval_diagrams_year() {
   global $period, $Year, $Month, $Day, $MonthsNames, $line, $Year_interval, $Query, $ResultArray, $FontSize, $namber_base ,$y_positions;
      if($_COOKIE["period"] = 1) {        
        $begin = $_COOKIE['begin'];
        $end = $_COOKIE['end'];
        ...
        for ($i = 1; $i <= 2; $i++) {        
          $Query[$i] = "SELECT YEAR( OrderDate) AS label, SUM( M_All ) AS Total FROM $namber_base[$i] WHERE YEAR(OrderDate) >= $year_begin and YEAR(OrderDate)<= $year_end GROUP BY YEAR( OrderDate) ORDER BY YEAR(OrderDate)";        
...
function construction_interval_diagrams_month() {
    global $period, $Year, $Month, $Day, $MonthsNames, $MonthsNames_double, $FontSize, $line, $Year_interval, $Query, $ResultArray, $namber_base, $y_positions;
      if($_COOKIE["period"] = 2 ) { 
        $period = 'monthly';       
        ...
        for ($i = 1; $i <= 2; $i++) {        
          $Query[$i] = "SELECT convert(char(6), OrderDate, 112) AS label,  SUM( M_All ) AS Total FROM $namber_base[$i] WHERE (OrderDate BETWEEN '$begin' AND '$end' ) AND NOT OrderDate IN ('$end') GROUP BY convert(char(6), OrderDate, 112) ORDER BY convert(char(6), OrderDate, 112)";                   
...
function construction_interval_diagrams_day() {
   global $period, $FontSize, $number_of_days, $line_link, $Query, $ResultArray, $namber_base, $day_interval, $MonthsNames_double_for_day, $y_positions;
      if($_COOKIE["period"] = 3 ) { 
        ...
        for ($i = 1; $i <= 2; $i++) {        
          $Query[$i] = "SELECT convert(char(8), OrderDate, 112) AS label, SUM( M_All ) AS Total FROM $namber_base[$i] WHERE OrderDate BETWEEN '$begin' AND '$end' GROUP BY convert(char(8), OrderDate, 112) ORDER BY convert(char(8), OrderDate, 112)";                   
...
function construction_diagrams() {
   global $period, $Year, $Month, $Day, $MonthsNames, $namber_base, $line, $first_year, $Year_interval, $last_year, $line_link, $Query, $ResultArray, $number_of_days, $FontSize, $y_positions;
     switch($period)
       ...
       case 'yearly':
          for ($i = 1; $i <= 2; $i++) {
             $Query[$i] = "SELECT YEAR( OrderDate) AS label, SUM( M_All ) AS Total FROM $namber_base[$i] WHERE YEAR(OrderDate) >=$first_year GROUP BY YEAR( OrderDate) ORDER BY YEAR(OrderDate)";        
             $ResultArray[$i] = array_fill($first_year, 8, 0); // fill the Result array with 0 values for each month
          } 
          $y_positions = '0'; 
          $FontSize = '11px';                  
...
if(isset($_COOKIE["interval_chart"]) AND $_COOKIE["interval_chart"] == 1 AND $_COOKIE["node_name"] === "node/10") {
            ...
            $Output .= '</categories>'; 
              for ($i = 1; $i <= 2; $i++) {               
                 $Output .= '<dataset seriesname="'.$line[$i].'">';                             
                   ...
                   $value = $value/1000;
                   $value = round($value, 2); 
                   $Output .= '<set value="'.$value.'" link="newchart-xmlurl-ob_all_chart.php?type=monthly&amp;year='.$yearly.'&amp;of_'.$i.'"/>';         
                   }     
        	     $Output .= '</dataset>';   	      
              } 
