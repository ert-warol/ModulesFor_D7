...
function construction_diagrams() {
   ...
   unset($ResultArray);         
   $ResultArray = array();               
   $Query = "SELECT OrderNo AS Value, OrderDate, OrderSm, M_1 AS Total 
             FROM otk_ru.ru_mtn.RudaS_WM_OF1
             WHERE OrderDate = '$today_all_date'  
             GROUP BY OrderSm, OrderDate, M_1, OrderNo 
...
function construct_diagram_speedometer(){ 
 global $value_speedometr, $min_value;
   ...
   $Output = '<chart theme="fint" caption="Mill # 11," subcaption="tn/h" baseChartMessageFontSize="14" baseFontSize="14" captionFontSize="14" subCaptionFontSize="14" captionFont="calibri" subCaptionFont="calibri" baseFont="calibri" captionfontcolor="#000000" subcaptionfontbold="10" bgcolor="#ffffff" showborder="0" lowerlimit="'.$min_value.'" upperlimit="190" numbersuffix="" valuefontsize="16" valuefontbold="0" gaugefillratio="40,20,40" refreshinterval="180" targetThickness="4">';
   $Output .= '<colorrange>';
   $Output .= '<color minvalue="'.$min_value.'" maxvalue="210" code="#e44a00" alpha="25" />';
   $Output .= '<color minvalue="210" maxvalue="240" code="#f8bd19" alpha="25" />';
   $Output .= '<color minvalue="240" maxvalue="270" code="#6baa01" alpha="25" />';
   $Output .= '</colorrange>';  
   $Output .= '<value FontSize="18">'.$value_speedometr.'</value>'; 
...
$Query = "SELECT IDRecNo, F_Q_1 AS Total FROM otk_ru.ru_mtn.WM_OF1 WHERE IDRecNo = 1";
