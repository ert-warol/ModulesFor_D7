...
function construction_diagrams() {
   ...
   unset($ResultArray);         
   $ResultArray = array();               
   $Query = "SELECT OrderNo AS Value, OrderDate, OrderSm, M_151 AS Total 
             FROM otk_ru.ru_mtn.RudaS_WM_OF2
             WHERE OrderDate = '$today_all_date'  
             GROUP BY OrderSm, OrderDate, M_151, OrderNo 
...
function construct_diagram_speedometer(){ 
 global $value_speedometr, $min_value;
   ...
   $Output = '<chart theme="fint" caption="Mill # 151," subcaption="tn/h" captionFont="calibri" subCaptionFont="calibri" baseFont="calibri" baseFontSize="14" captionFontSize="14" subCaptionFontSize="14" captionfontcolor="#000000" subcaptionfontbold="10" bgcolor="#ffffff" showborder="0" lowerlimit="'.$min_value.'" upperlimit="190" numbersuffix="" valuefontsize="16" valuefontbold="0" gaugefillratio="40,20,40" refreshinterval="180" targetThickness="4">';
   $Output .= '<colorrange>';
   $Output .= '<color minvalue="50" maxvalue="110" code="#e44a00" alpha="25" />';
   $Output .= '<color minvalue="110" maxvalue="140" code="#f8bd19" alpha="25" />';
   $Output .= '<color minvalue="140" maxvalue="190" code="#6baa01" alpha="25" />';
   $Output .= '</colorrange>';  
...
$Query = "SELECT IDRecNo, F_Q_11 AS Total FROM otk_ru.ru_mtn.WM_OF2 WHERE IDRecNo = 1";
