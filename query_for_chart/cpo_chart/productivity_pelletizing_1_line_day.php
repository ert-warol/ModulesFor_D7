   ...
   global $period, $Month, $Day, $MonthsNames,  $Query, $ResultArray, $number_of_days, $y_positions, $today_year, $today_month, $ChartHeading, $XaxisName, $today_all_date;
     switch($period) {
       default:
         ...
         case 'daily':
            $Query = "SELECT DAY( Dat ) AS Value, SUM( PrLine1 ) AS Total FROM cpo.dbo.RsProizvCPO WHERE YEAR( Dat )= $today_year AND MONTH( Dat )= $today_month  GROUP BY DAY( Dat ) ORDER BY DAY( Dat) ";
         ...
         break;
         case 'hourly':
            $Query = "SELECT Chas AS Value, SUM( PrLine1 ) AS Total FROM cpo.dbo.RsProizvCPO WHERE YEAR( Dat )= $today_year AND MONTH( Dat )= $Month AND DAY( Dat )= $Day
...
$Output = '<chart caption="'.$ChartHeading.'" subcaption= "" xaxisname="'.$XaxisName.'" baseFontSize="14" tickValueDistance="14" yAxisNameFont="Calibri" captionFont="Calibri" valueFont="Calibri" subCaptionFont="calibri" baseFont="calibri" baseChartMessageFont="Calibri" rotateLabels="'.$y_positions.'" yaxisname="Productivity, thousand tons/day" numberSuffix="" numberScaleValue = "2" decimals="2" palettecolors="#246624" valuefontcolor="#ffffff" bgcolor="#ffffff" showborder="0" basefont="Helvetica Neue,Arial" captionfontsize="14" subcaptionfontsize="14" subcaptionfontbold="0" placevaluesinside="1" rotatevalues="1" showshadow="0" divlinecolor="#999999" divlinedashed="1" divlinethickness="1" divlinedashlen="1" divlinegaplen="1" canvasbgcolor="#ffffff">';
...
switch($period)
{
    ...
    case 'daily':  
        ...
        foreach($ResultArray as $DayNumber => $value) {  // DayNumber is day (1-31)
            $value = round($value, 2);
            $value = $value/1000;  
            $Output .= '<set label="'.$DayNumber.'" value="'.$value.'" link="newchart-xmlurl-productivity_pelletizing_1_line_day.php?type=hourly&amp;year='.$today_year.'&amp;month='.$today_month.'&amp;day='.$DayNumber.'"/>';
            $i++;
        }
        break;
    case 'hourly':   
