...
function construction_diagrams() {
   global $period, $Year, $Month, $Day, $ChartHeading, $XaxisName, $MonthsNames, $namber_base, $line, $first_year, $Year_interval, $last_year, $line_link, $Query, $ResultArray, $number_of_days, $FontSize, $y_positions, $today_m, $today_year_all, $today_all_date, $today_year, $today_all;
          ...
          unset($ResultArray);         
          $ResultArray = array();               
          $Query = "SELECT Dat, Chas AS label, PrLine3 AS Total
                    FROM cpo.dbo.RsProizvCPO
                    WHERE Dat = '$today_all_date'
                    GROUP BY Dat, Chas, PrLine3";
          ...
          $y_positions = '0'; 
          $FontSize = '14px';                  
          $ChartHeading = 'The Line # 3 productivity for '.$today_all_date;
          $XaxisName = 'Hours';      
}
...
//Generate Chart XML: Main Body
            ...
            for($j = 0; $j <= 23; $j++) {
               ...
               $Output .= '</categories>';            
               $Output .= '<dataset seriesname="Line 3" alpha="100" anchorRadius="4" valuePosition="BELOW">';                             
                   foreach($ResultArray as $yearly => $value) {                                        
                   $value = round($value, 0); 
                   if($value == NULL) {
                     unset($value);
