   ...
   global $period, $Year, $Month, $Day, $MonthsNames,  $first_year, $Year_interval, $last_year,  $Query, $ResultArray, $number_of_days, $FontSize, $y_positions, $today_year, $today_month, $ChartHeading, $XaxisName;
     switch($period) {
       default:
         ...
         case 'monthly':
            $Query = "SELECT MONTH( Dat ) AS Value, SUM( PrLine1 ) AS Total FROM cpo.dbo.RsProizvCPO WHERE YEAR(Dat)=$today_year GROUP BY MONTH( Dat ) ORDER BY MONTH(Dat) ";
         ...
         break;
         case 'daily':
            $Query = "SELECT DAY( Dat ) AS Value, SUM( PrLine1 ) AS Total FROM cpo.dbo.RsProizvCPO WHERE YEAR( Dat )=$today_year AND MONTH( Dat )=$Month GROUP BY DAY( Dat ) ORDER BY DAY( Dat) ";
...
switch($period)
{
    ...
    case 'monthly':
        ...
        foreach($ResultArray as $MonthNumber => $value) {  // MonthNumber is month number (1-12)
            $value = round($value, 2);
            $value = $value/1000;
            $Output .= '<set label="' .$MonthsNames[$MonthNumber]. '" value="' .$value. '" link="newchart-xmlurl-productivity_pelletizing_1_line_month.php?type=daily&amp;year=' .$today_year. '&amp;month=' .$MonthNumber. '"/>';
        }
        break;
    ...
    case 'daily':  
        ...
        foreach($ResultArray as $DayNumber => $value) {  // DayNumber is day (1-31)
            $value = round($value, 2);
            $value = $value/1000;  
            $Output .= '<set label="' .$DayNumber. '" value="' .$value. '" link="newchart-xmlurl-productivity_pelletizing_1_line_month.php?type=hourly&amp;year=' .$Year. '&amp;month=' .$Month. '&amp;day=' .$DayNumber. '"/>';
            $i++;
        }
        break;
    case 'hourly':   
