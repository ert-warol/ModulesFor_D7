   ...
   global $period, $Month, $Day, $MonthsNames,  $Query, $ResultArray, $number_of_days, $y_positions, $today_year, $today_month, $ChartHeading, $XaxisName, $today_all_date;
     switch($period) {
       default:
         ...
         case 'daily':
            $Query = "SELECT DAY( Dat ) AS Value, SUM( PrLine2 ) AS Total FROM cpo.dbo.RsProizvCPO WHERE YEAR( Dat )= $today_year AND MONTH( Dat )= $today_month  GROUP BY DAY( Dat ) ORDER BY DAY( Dat) ";
...
switch($period)
{
    ...
    case 'daily':  
        ...
        foreach($ResultArray as $DayNumber => $value) {  // DayNumber is day (1-31)
            $value = round($value, 2);
            $value = $value/1000;  
            $Output .= '<set label="'.$DayNumber.'" value="'.$value.'" link="newchart-xmlurl-productivity_pelletizing_2_line_day.php?type=hourly&amp;year='.$today_year.'&amp;month='.$today_month.'&amp;day='.$DayNumber.'"/>';
            $i++;
        }
        break;
    case 'hourly':   
