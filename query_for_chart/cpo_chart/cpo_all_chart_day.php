...
function construction_diagrams() {
   global $period, $Year, $Month, $Day, $MonthsNames, $line_link, $Query, $ResultArray, $number_of_days, $FontSize, $y_positions, $today_year, $today_month, $today_day;
     switch($period) {
       ...
       case 'daily':
         for ($i = 1; $i <= 4; $i++) {           
            $Query[$i] = "SELECT DAY( Dat ) AS label, SUM( $line_link[$i] ) AS Total FROM  cpo.dbo.RsProizvCPO WHERE YEAR( Dat )= $today_year AND MONTH( Dat )= $today_month GROUP BY DAY( Dat ) ORDER BY DAY(Dat)";
             ...
             for ($i = 1; $i <= 4; $i++) {               
                $Output .= '<dataset seriesname="'.$line[$i].'">';                             
                  foreach($ResultArray[$i] as $DayNumber => $value) {
                     $value = $value/1000;
                     $value = round($value, 2);
                     $Output .= '<set value="'.$value.'" link="newchart-xmlurl-cpo_all_chart_day.php?type=hourly&amp;year='.$today_year.'&amp;month='.$today_month.'&amp;day='.$DayNumber.'&amp;number_line='.$line_link[$i].'"/>';         
                  }     
              $Output .= '</dataset>';   	      
              }
