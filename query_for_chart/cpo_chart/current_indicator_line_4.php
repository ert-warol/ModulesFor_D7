...
function construction_diagrams() {
   ...
   unset($ResultArray);         
   $ResultArray = array();               
   $Query = "SELECT Dat, Chas AS label, PrLine4 AS Total
                    FROM cpo.dbo.RsProizvCPO
                    WHERE Dat = '$today_all_date'
                    GROUP BY Dat, Chas, PrLine4";
...
function construct_diagram_speedometer(){ 
 global $value_speedometr, $min_value;
   ...
   $Output = '<chart theme="fint" caption="Line # 4," subcaption="tn/h" captionFont="calibri" subCaptionFont="calibri" baseFont="calibri" baseFontSize="14" captionFontSize="14" subCaptionFontSize="14" captionfontcolor="#000000" subcaptionfontbold="10" bgcolor="#ffffff" showborder="0" lowerlimit="0" upperlimit="450" numbersuffix="" valuefontsize="20" valuefontbold="0" gaugefillratio="40,20,40" refreshinterval="180" targetThickness="4">';
   $Output .= '<colorrange>';
...
$Query = "SELECT a.*, PrLine4 as Total                
