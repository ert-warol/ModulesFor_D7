...
$today_day = date('d');
$today_month = date('m');
$namber_value = array(NULL, 'G1', 'G1-1', 'G2' );
$color_line = array(NULL, '#F72B08', '#246624', '#0000FF'); 
if ($today_month <=10) {
...
function construction_diagrams() {
   global $ChartHeading, $XaxisName, $base_date, $Query, $ResultArray, $FontSize, $y_positions, $today_all_date;
          ...
          $ResultArray[1] = array_fill(1, 504, 0);  
          
          $Query[2] = "SELECT Chas, Line, (convert(char(8), Dat, 112)+Chas) AS label, (L1 + L2 + L3 + L4)  AS Total 
                    FROM cpo_svod.dbo.LineRsH 
                    WHERE (Dat BETWEEN '$base_date' AND '$today_all_date' ) AND Line = 1
                    GROUP BY Line, Dat, Chas, L1, GazObg, L2, L3, L4  
                    ORDER BY Dat, Chas";              
                        
          $ResultArray[2] = array_fill(1, 504, 0); 
          $Query[3] = "SELECT Chas, Line, (convert(char(8), Dat, 112)+Chas) AS label, (GazObg*1000) AS Total 
                    FROM cpo_svod.dbo.LineRsH 
                    WHERE (Dat BETWEEN '$base_date' AND '$today_all_date' ) AND Line = 1
                    GROUP BY Line, Dat, Chas, L1, GazObg  
                    ORDER BY Dat, Chas";              
                        
          $ResultArray[3] = array_fill(1, 504, 0);
          
          $y_positions = '0'; 
          $FontSize = '14px';                  
          $ChartHeading = 'Gas consumption period : '.$base_date.' '.$today_all_date;
...
$Output ="<chart caption='$ChartHeading' xaxisname='$XaxisName' baseFontSize='14' tickValueDistance='14' yAxisNameFont='Calibri' captionFont='Calibri' valueFont='Calibri' baseChartMessageFont='Calibri' labelheight='70' yAxisMinValue='-10' yAxisMaxValue='7000' rotateLabels='1' yaxisname='Gas consumption m3/h' xAxisNameFontSize='14' yAxisNameFontSize='14' thousandSeparatorPosition='0' numberScaleValue = '1' showlabels='1' showvalues='1' decimals='1' formatNumberScale='0' FormatNumber='0' numberprefix='' numberSuffix=' ' placevaluesinside='1' rotatevalues='1' valueFontSize='$FontSize' bgcolor='FFFFFF' legendshadow='0' legendborderalpha='50' canvasborderthickness='1' canvasborderalpha='50' palettecolors='#246624' showborder='0' theme='fint'>"; 
...
//Generate Chart XML: Main Body
            ...
            $Output .= '</categories>'; 
            for ($i = 1; $i <= 3; $i++){           
               $Output .= '<dataset seriesname="'.$namber_value[$i].'" color="'.$color_line[$i].'" alpha="50" drawAnchors="0">';                             
