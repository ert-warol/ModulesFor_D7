      ...
      for ($i = 1; $i <=3; $i++) {
         ...
         $Query[$i] = "DECLARE @start_date datetime, @end_date datetime
             ...
             SELECT CONVERT(varchar, tab1.OrderDate, 104) as Value, tab1.Ruda,
           	 CASE WHEN sK25 > 0 THEN sFe/sK25 ELSE 0 END as $label_fe,	           
	         CASE WHEN sK25 > 0 THEN sFeKNC/sK25 ELSE 0 END as $label_fekn,
	         CASE WHEN sK25 > 0 THEN sFeMg/sK25 ELSE 0 END as $label_FeMg               
             FROM
            ( 
	         SELECT MAX(OrderDate) as OrderDate, MAX(Ruda) as Ruda, 
		     SUM(Fe*Tonn_K25) as sFe,		           
		     SUM(FeKNC*Tonn_K25) as sFeKNC,
		     SUM(FeMg*Tonn_K25) as sFeMg,               
		     SUM(Tonn_K25) as sK25                      
	         FROM
	        (
	         SELECT OrderDate, Ruda, Fe, FeMg, FeKNC, Tonn_K25 as Tonn_K25
             FROM [otk_ru].[ru_mtn].[Priem_Ekg_Q] 
	         WHERE ((OrderDate >= @start_date) AND (OrderDate <= @end_date)) AND (Ruda = 'K2/5') 
	         AND (Tonn_K25 > 0) AND ((Fe > 0) AND (FeKNC > 0) AND (FeMg > 0)) 	  
	        ) tab
          ...
          $y_positions = '0'; 
          $FontSize = '11px';                  
          $ChartHeading = 'Chemical analysis of ore K2/5 Fe, FeKNC, FeMg: from '.$today_begin.' to '.$today_all_date;
          $XaxisName = 'Daily rate';
      
}
function construction_diagrams_interval () {
   global $MonthsNames, $first_year, $Year_interval, $last_year, $line_link, $Query, $ResultArray, $FontSize, $y_positions, $today_m, $today_year_all, $today_all_date, $today_year, $ChartHeading, $XaxisName, $range_days;
   unset($ResultArray);         
   $ResultArray = array();                    
      for ($i = 1; $i <=3; $i++) {
         if($i == 1): {
        ...
        $end = date("Y-m-d", strtotime($_COOKIE['end_interval_day']));
         ...
         $Query[$i] = "DECLARE @start_date datetime, @end_date datetime
          ...
          $y_positions = '0'; 
          $FontSize = '11px';                  
          $ChartHeading = 'Chemical analysis of ore K2/5 Fe, FeKNC, FeMg: from '.$begin.' to '.$end;
          $XaxisName = 'Daily rate'; 
}
...
$Output ='<chart exportenabled="1" exportAtClientSide="1" legendItemFontSize="14" slantlabels="1" rotateLabels="1" caption="'.$ChartHeading.'" xaxisname="'.$XaxisName.'" yaxisname="chemical analysis of ores" showvalues="0" palettecolors="#246624,#F72B08,#2207F3" theme="fint">'; 
