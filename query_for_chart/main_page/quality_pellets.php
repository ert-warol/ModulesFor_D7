...
$day_interval = array(); 
$Year_interval = array();
$seriesname = array(null, 'Pelletizing 1/1m', 'Pelletizing 1/2m', 'Pelletizing 2/3m', 'Pelletizing 2/4m',);
...
if($today_day == 1 AND $today_month == 1){
  ...
  $Query[1] = "SELECT OrderSm, IndexChas, NumChas AS label, OrderDate, Obg1_Fe AS Total
  ...
  $Query[2] = "SELECT OrderSm, IndexChas, NumChas AS label, OrderDate, Obg2_Fe AS Total
             FROM otk_ru.ru_mtn.HimA_CPO1
             WHERE DAY(OrderDate) = $last_day AND MONTH(OrderDate) = $month_of_the_previous_day AND YEAR(OrderDate) = $year_of_the_previous_day
             ORDER BY OrderSm, IndexChas";
  
  $Query[3] = "SELECT OrderSm, IndexChas, NumChas AS label, OrderDate, Obg3_Fe AS Total
             FROM otk_ru.ru_mtn.HimA_CPO2
             WHERE DAY(OrderDate) = $last_day AND MONTH(OrderDate) = $month_of_the_previous_day AND YEAR(OrderDate) = $year_of_the_previous_day
             ORDER BY OrderSm, IndexChas";
  $Query[4] = "SELECT OrderSm, IndexChas, NumChas AS label, OrderDate, Obg4_Fe AS Total
             FROM otk_ru.ru_mtn.HimA_CPO2
             WHERE DAY(OrderDate) = $last_day AND MONTH(OrderDate) = $month_of_the_previous_day AND YEAR(OrderDate) = $year_of_the_previous_day
             ORDER BY OrderSm, IndexChas";     
                       
  $ChartHeading = 'Pellets quality: from '.$today_all;
  $XaxisName = 'Hours';
